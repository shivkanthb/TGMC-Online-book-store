...
<form class="form_settings" method="post" action="AdminAuthorReports.jsp" id="form1">
...
<% int xx=0; %>
	...
	%>
			...
			String query = " SELECT AUTHOR.AUTHORID,FIRSTNAME,LASTNAME,SUM(QUANTITY) FROM AUTHOR,AUTH_BOOKS,TRANSACTION WHERE BILLID IN (SELECT BILLID FROM BILL WHERE BILL_DATE=?) AND AUTH_BOOKS.ISBN=TRANSACTION.ISBN AND AUTHOR.AUTHORID=AUTH_BOOKS.AUTHID GROUP BY FIRSTNAME,LASTNAME,AUTHOR.AUTHORID";
			...
			float maintotal = 0;
			String isbn="";
			String title="";boolean first=false;
			while (rs.next()) {
				
				isbn=rs.getString(1);
				title=rs.getString(2)+" "+rs.getString(3);
				int qty = Integer.parseInt(rs.getString(4));
				//float total=qty*(Float.parseFloat(rs.getString(4))*(1-Float.parseFloat(rs.getString(5))/100));
				//maintotal+=total;
	if(!first)
	{	%>
