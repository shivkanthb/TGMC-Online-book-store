...
query = "SELECT * from BOOKS where TITLE=?";
	...
	{
query = "SELECT * FROM BOOKS WHERE ISBN IN (SELECT ISBN FROM BOOK_CAT WHERE CAT=(SELECT CAT FROM CATEGORY WHERE NAME=?))";
	}
	if(y.equals("author"))
	{
query = "SELECT * FROM BOOKS WHERE ISBN IN(SELECT ISBN FROM AUTH_BOOKS WHERE AUTHID=(SELECT AUTHORID FROM AUTHOR WHERE FIRSTNAME=?))";
	}
	if(y.equals("publisher"))
	{
query = "SELECT * FROM BOOKS WHERE PUBID IN(SELECT PUBID FROM PUBLISHER WHERE NAME=?)";
	}
...
%>
	...
	ResultSet rs=null;
		...
		<img src='images/<%=resultset.getString(10) %>'></img><br/>
